...
class MultiScaleFeaturePatchTST(nn.Module):
    """
    多尺度多特征融合的PatchTST模型用于轴承故障分类
    ...
    def __init__(self, input_channels=3, seq_length=1000, num_classes=38,
        ...
        """
        super(MultiScaleFeaturePatchTST, self).__init__()
        ...
        num_feature_types = sum([1, use_time_features, 
                                use_fft, bool(use_fft and True),  # FFT特征投影 + 原始FFT分析
                                use_wavelet])
...
class MultiScaleFeaturePatchTSTClassifier(nn.Module):
    """
    封装MultiScaleFeaturePatchTST模型，提供与其他分类器兼容的接口
    """
    def __init__(self, input_channels=3, seq_length=1000, num_classes=38,
                 patch_sizes=[16, 32, 64], strides=[8, 16, 32], d_model=128, n_heads=8, 
                 num_layers=3, dropout_rate=0.1, use_fft=True, use_wavelet=True, 
                 use_time_features=True, sampling_rate=10000):
        super(MultiScaleFeaturePatchTSTClassifier, self).__init__()
        
        self.model = MultiScaleFeaturePatchTST(
