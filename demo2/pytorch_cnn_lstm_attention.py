...
class CNNLSTM_Attention(nn.Module):
    ...
    def __init__(self, input_channels=3, seq_length=1000, num_classes=38, 
        ...
        super(CNNLSTM_Attention, self).__init__()
        ...
        # 双向LSTM层
        self.lstm = nn.LSTM(filters*4, lstm_hidden, batch_first=True, bidirectional=True)
        self.dropout1 = nn.Dropout(dropout_rate)
        
        # 自注意力层
        self.attention = SelfAttention(lstm_hidden*2)  # 因为是双向LSTM，所以维度是lstm_hidden*2
            ...
            for inputs, labels in val_loader:
                ...
                if 'tqdm' not in globals():
                    if batch_count % 5 == 0 or batch_count == total_val_batches:
                        ...
                        print(f"\r验证进度: [{batch_count}/{total_val_batches}] {progress:.1f}%", end="")
...
def visualize_attention_weights(model, test_loader, device, num_samples=3):
    ...
    for i in range(num_samples):
        ...
        # 假设attention是LSTM之后的注意力，长度可能是原始信号的1/4
